...
" - Also allow a [count] for `r to select (or query for) a mark group, as
"   with `m.
" - CHG: Also set the current mark to the used mark group when a mark was set
"   via `r and :Mark so that it is easier to determine whether the
